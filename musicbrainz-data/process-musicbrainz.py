...
for i in range(0, 25, 25):
    ...
    if response.status_code != 200:
        print("stopped receiving responses at track_id {}".format(unique_tracks[i]))
        break
    for track_id in response.json():
        try:
            features = response[track_id]['0']['highlevel']
...
song_feats.to_csv('song_features.csv')
